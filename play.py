...
def browse_prompts():
    ...
    while True:
        print("\n" * 3)
        dirinsides = sorted(os.listdir(path))
        
        for file in dirinsides: #remove all files that are not folders/txt
            if "." in file and not file.endswith(".txt"):
                dirinsides.remove(file)
       
        for enum, file in enumerate(dirinsides):
            if "." not in file:
                file += "/" #in case file is a folder
            elif file.endswith(".txt"):    
                file = file.replace(".txt", "") 
            print(f"{enum}) {file}")
        if path != "./prompts/":
            print(f"{len(dirinsides)}) Back")
        
        try:
            dir_prompt = int(input("Choose prompt/dir:"))
        except ValueError:
            print("This is not a number.")
            continue
        
        if dir_prompt == len(dirinsides) and path != "./prompts/":
            splitpath = path.split("/")
            path = "/".join(splitpath[:-2]) #go back to previous folder
            path += "/" #add trailing slash for future operations on path
            continue
        
        try:
            choice = dirinsides[dir_prompt]
        except IndexError:
            print("Wrong choice. No such amount of choices.")
            continue
            
        if choice.endswith(".txt"):
            with open(path + dirinsides[dir_prompt], "r") as prompt:
                context = prompt.readline().replace("\n", "") #read first two lines
                promptt = prompt.readline().replace("\n", "")
                return context, promptt
        elif "." not in choice:
            path += choice + "/"
        else:
...
def check_variables(to_check):
    matches = re.findall("[$]{.*?}", to_check) #find all patterns that match ${x}
    if matches:
        variables = []
        for match in matches:
            if match not in variables:
                user_ask = input(match[2:-1] + " ==> ") #to clarify the input to user, present him the same variable from context but without brackets
                to_check = to_check.replace(match, user_ask) #replace the variable by user input
                variables.append(match) #append it to list of variables so we don't replace it again
    return to_check

def get_custom_prompt():
    choice = input("Write your own prompt or choose from folder? 1 for choose, 2 for writing custom.\nChoice: ")
    if choice == "2":
        console_print(
        ...
        selected = browse_prompts()
        context, prompt = check_variables(selected[0]), check_variables(selected[1])
        if len(context) > 0 and not context.endswith(" "):
            context = context + " "
        return context, prompt
    ...
    while True:
        ...
        while story_manager.story is None:
            ...
            if splash_choice == "new":
                ...
                world_info = input("Have any world info to gib, m8? Just input the name of json file you've made! ")
                ...
                if world_info.lower():
                    with open(f"./worldinfo/{world_info}.json", "r") as wi_json:
                        data = list(json.load(wi_json))
                        story_manager.world_info = data
                        story_manager.wi_path = f"./worldinfo/{world_info}.json"
                else:
                    story_manager.world_info = None
                    story_manager.wi_path = None
                ...
                if command == "reset":
                    ...
                    if len(story_manager.story.actions) > 0:
                        ...
                        try:
                            ...
                            if ping:
                                PlaySound("ping.mp3", SND_FILENAME)
                    else:
