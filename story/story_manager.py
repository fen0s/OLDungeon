...
class StoryManager:
    ...
    def load_from_storage(self, story_id, decrypt=True):
        ...
        if exists:
            ...
            if "wi_path" in game.keys():
                with open(game["wi_path"], "r") as wi_json:
                    data = list(json.load(wi_json))
                    self.world_info = data
                    self.wi_path = game["wi_path"]
            if changed:
    ...
    def save_story(self, name=None, overwrite=True):
        ...
        story_dict["wi_path"] = self.wi_path
    ...
    def parse_wi(self, action):
        try:
            if self.world_info:
                action_clean = action.replace(".", "").replace("!", "").replace(",", "").replace("?", "").lower()
                for entry in self.world_info:
                    for key in entry.get("keywords"):
                        if key in action_clean:
                            if entry.get("context") not in self.story.temp_context:
                                self.story.temp_context += entry.get("context") + "\n"
                                break
...
class UnconstrainedStoryManager(StoryManager):
    ...
    def generate_result(self, action):
        self.parse_wi(action)
        if self.story.temp_context:
            self.story.temp_context = '\n' + self.story.temp_context
